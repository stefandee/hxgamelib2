...
class LeaderboardEntry implements IXML
{
  ...
  public function fromXML(elem : flash.xml.XML) : Void
  {
    ...
    playerName        = elem.elements("player_name")[0].toString();
    playerCountryCode = elem.elements("code")[0].toString();
    value             = Std.parseInt(elem.elements("value")[0].toString());
    timeStamp         = Std.parseInt(elem.elements("timestamp")[0].toString());
  }
}
...
class Leaderboard extends Observable, implements IXML
{
  ...
  public function fromXML(elem : flash.xml.XML) : Void
  {
    ...
    var items = elem.elements("item");

    for(i in 0...items.length())
    {
      var entry = new LeaderboardEntry();

      entry.fromXML(items[i]);
