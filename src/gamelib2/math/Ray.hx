...
class Ray
{
                               ...
                               store : Vector3f, doPlanar : Bool, quad : Bool ) : Bool
  {
    ...
    if (dirDotNorm > MathEx.FLT_EPSILON) 
    {
      sign = 1.0;
    } 
    else if (dirDotNorm < -MathEx.FLT_EPSILON) 
    {
      sign = -1.0;
  ...
  public function intersectsWherePlane(p : Plane, loc : Vector3f) : Bool
  {
    ...
    if (denominator > -MathEx.FLT_EPSILON && denominator < MathEx.FLT_EPSILON)
    {
