...
class Sentry extends Spatial
{
  ...
  */
  ...
  public function drawAFrame(buffer : flash.display.BitmapData, animIndex : Int, aframeIndex : Int, cameraMatrix : Matrix3f)
  {
    ...
    for(fModule in template.frames[aframeTmpl.frameIndex].fModules)
    {
      ...
      {        
        ...
        //buffer.copyPixels(moduleTmpl.bitmapData, cachedFlashRect, cachedFlashPoint, null, null, true);
        //buffer.merge(moduleTmpl.bitmapData, cachedFlashRect, cachedFlashPoint, 1, 1, 1, Std.int(alpha * 255));
      ...
      else
      {
        ...
        */
        ...
        buffer.draw(moduleTmpl.bitmapData, moduleMatrixFlash, colorTransform, flash.display.BlendMode.NORMAL, null, true);
      }
    }
  }
  ...
  private override function setAlpha(v : Float) : Float
  {
  ...
  private function hasPixelPerfectCollision(spatialSentry : Sentry) : Bool
  {
    ...
    */
    ...
    if (srcBitmapData.hitTest(cachedFlashPoint, 0xFF, dstBitmapData, cachedFlashPoint, 0xFF))
    {
  ...
  public override function pickComp(x : Float, y : Float, ?requiredOnBits : Int = 1) : Bool
  {
    ...
    for(fModule in template.frames[aframeTmpl.frameIndex].fModules)
    {
      ...
      //var alphaComp = (moduleTmpl.bitmapData.getPixel32(Std.int(pickV2.x), Std.int(pickV2.y)) & 0xFF000000) >> 24;
      ...
      //trace("color: " + moduleTmpl.bitmapData.getPixel32(Std.int(pickV2.x), Std.int(pickV2.y)));
      //trace(alphaComp);
