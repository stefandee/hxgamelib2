...
{  
  public var playerName (default, setPlayerName)       : String;
  public var submitEntries (default, setSubmitEntries) : Array<SubmitEntry>;
  ...
  */
	...
	public function update(type : Int, source : Observable, userData : Dynamic) : Void
  {
    ...
    if (source == api.submit)
    {
      ...
      switch(type)
      {
        case gamelib2.leaderboard.Submit.SUCCESS_EVENT:
        {
        }

        case gamelib2.leaderboard.Submit.ERROR_EVENT:
        {
        }
      }
    }    
  ...
  private function setPlayerName(v : String) : String
  {
  ...
  private function setSubmitEntries(v : Array<SubmitEntry>)
  {
    if (v != null)
    {
      submitEntries = new Array();
